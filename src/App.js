...
const lesson = {
  ...
  // Vocabulary Task 1: Match word to definition (words 1-5)
  vocabTask1: {
    ...
    answers: ["c", "d", "b", "e", "a"], // correct letter for each word
  },
  // Vocabulary Task 2: Fill in the gaps
  vocabTask2: {
  ...
  paragraphs: [
    ...
    },
    {
      ...
      task: {
        type: "mixed_p5", label: "Task 5 — MULTIPLE CHOICE",
        questions: [
...
// ─── VOCAB TASK 1: Match word to definition ───────────────────────────────────
function VocabMatchTask({ task, onDone }) {
  const [selections, setSelections] = useState({}); // { wordIndex: letter }
  const [checked, setChecked] = useState(false);
  const allSelected = Object.keys(selections).length === task.words.length;

  const handleSelect = (wordIdx, letter) => {
    if (checked) return;
    setSelections(prev => ({ ...prev, [wordIdx]: letter }));
  };
    ...
    <Card>
      ...
      <div style={{ background: C.surface, borderRadius: 12, padding: "12px 16px", marginBottom: 16 }}>
        {task.definitions.map(d => (
          <div key={d.letter} style={{ fontSize: 13, color: C.text, marginBottom: 6, lineHeight: 1.5 }}>
            <span style={{ fontWeight: 700, color: C.accent }}>{d.letter}.</span> {d.text}
          </div>
        ))}
      </div>

      {/* Words with letter selector */}
      {task.words.map((word, i) => {
        const sel = selections[i];
        const isCorrect = checked ? sel === task.answers[i] : null;
        return (
          <div key={i} style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 10, padding: "12px 16px", background: C.surface, borderRadius: 12, border: `1px solid ${checked ? (isCorrect ? C.success + "50" : C.error + "50") : C.accentBorder}` }}>
            <div style={{ fontWeight: 700, color: C.text, fontSize: 14, minWidth: 130 }}>{i + 1}. {word}</div>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {task.definitions.map(d => (
                <button key={d.letter} onClick={() => handleSelect(i, d.letter)} style={{
                  width: 32, height: 32, borderRadius: "50%",
                  background: sel === d.letter ? C.accent : C.card,
                  color: sel === d.letter ? "#0f1923" : C.textMuted,
                  border: `1.5px solid ${sel === d.letter ? C.accent : C.accentBorder}`,
                  fontWeight: 700, fontSize: 13, cursor: checked ? "default" : "pointer",
                  fontFamily: "'Segoe UI', sans-serif"
                }}>{d.letter}</button>
              ))}
            </div>
            {checked && (
              <div style={{ fontSize: 13, color: isCorrect ? C.success : C.error, marginLeft: "auto" }}>
                {isCorrect ? "✓" : `✗ → ${task.answers[i]}`}
              </div>
            )}
          </div>
        );
      ...
      {checked && (
        <div style={{ marginTop: 14 }}>
          <div style={{ padding: "10px 14px", borderRadius: 10, background: C.accentSoft, border: `1px solid ${C.accentBorder}`, fontSize: 13, color: C.text, marginBottom: 12 }}>
            ✅ Score: {score}/{task.words.length}
          </div>
          <Btn onClick={() => onDone(score, task.words.length)} style={{ width: "100%" }}>Continue to Vocabulary Task 2 →</Btn>
        </div>
      )}
    </Card>
  );
    ...
    <Card>
      ...
      {checked && (
        <div style={{ marginTop: 14 }}>
          ...
          <Btn onClick={() => onDone(score, total)} style={{ width: "100%" }}>Continue to Knowledge Check →</Btn>
        </div>
      )}
    </Card>
  );
  ...
  return (
    <div>
      ...
      {allDone && (
        <div style={{ marginTop: 14 }}>
          <div style={{ padding: "10px 14px", borderRadius: 10, background: C.accentSoft, border: `1px solid ${C.accentBorder}`, fontSize: 13, color: C.text, marginBottom: 12 }}>
            ✅ Score: {Object.values(results).filter(r => r.ic).length}/{total}
          </div>
...
function StudentPanel({ user, onLogout }) {
  // preStep: 0=vocab cards, 1=vocabTask1, 2=vocabTask2, 3=preCheck
  ...
  // Save any result to task_results — used for ALL stages including vocab tasks
  ...
  const handleVocabTask1Done = async (score, total) => {
    await saveTaskResult("pre", "Vocab Task 1 – Match", "Match word to definition (5 words)", `Score: ${score}/${total}`, `Score: ${total}/${total}`, score === total);
    setPreStep(2);
  };

  const handleVocabTask2Done = async (score, total) => {
    await saveTaskResult("pre", "Vocab Task 2 – Gap Fill", "Fill in the gaps (5 sentences)", `Score: ${score}/${total}`, `Score: ${total}/${total}`, score === total);
    setPreStep(3);
  };
      ...
      <div style={{ maxWidth: 700, margin: "0 auto", padding: "28px 20px" }}>
        ...
        {stage === 0 && (
          <div>
            ...
            {/* Step 1: Vocab Task 1 - Match */}
            {preStep === 1 && <VocabMatchTask task={lesson.vocabTask1} onDone={handleVocabTask1Done} />}

            {/* Step 2: Vocab Task 2 - Gap Fill */}
            {preStep === 2 && <VocabGapFillTask task={lesson.vocabTask2} onDone={handleVocabTask2Done} />}

            {/* Step 3: Pre-check MCQ */}
            {preStep === 3 && (
              <Card>
                <h3 style={{ marginBottom: 16, color: C.stage1 }}>🧠 Quick Knowledge Check</h3>
                <SingleMCQ
                  question={lesson.preCheck.question}
                  options={lesson.preCheck.options}
                  correct={lesson.preCheck.correct}
                  onResult={(sa, ca, ic) => handlePreCheckContinue(sa, ca, ic)}
                  disabled={false}
                />
              </Card>
            )}
...
function AdminPanel({ user, onLogout }) {
  ...
  const stageColor = (s) => s === "pre" ? C.stage1 : s === "while" ? C.accent : C.gold;
  const stageLabel = (s) => s === "pre" ? "Pre-Reading" : s === "while" ? "While Reading" : "Post-Reading";
              ...
              {attempts.map((att, ai) => (
                <div key={att.id} style={{ background: C.surface, borderRadius: 20, marginBottom: 28, border: `1px solid ${C.accentBorder}`, overflow: "hidden" }}>
                  ...
                  <div style={{ padding: "20px 24px" }}>
                    {/* All task results grouped by stage */}
                    {att.tasks.length > 0 && (
                      <div style={{ marginBottom: 24 }}>
                        <div style={{ fontSize: 12, color: C.textMuted, textTransform: "uppercase", letterSpacing: "0.08em", marginBottom: 14 }}>📊 All Task Results</div>
                        {att.tasks.map((tr, ti) => (
                    ...
                    {/* Open-ended post-reading answers */}

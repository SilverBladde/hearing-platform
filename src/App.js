...
// ─── LESSON DATA (exactly from the Word document) ─────────────────────────────
const lesson = {
  ...
  // Pre-reading check (knowledge activation)
  preCheck: {
    type: "mcq",
  ...
  // 5 paragraphs each with their exact tasks from the document
  paragraphs: [
    {
      emoji: "🥽",
      label: "Paragraph 1",
  ...
  postReading: [
    ...
    { id: "q2", question: "Using VR, students can explore… without… (complete using your own ideas)", hint: "Think about places students could visit virtually — rainforests, space, museums..." },
  ...
  return (
    <div style={{ marginBottom: 20 }}>
      ...
      {options.map((opt, i) => {
        ...
        if (checked) {
          if (i === correct) { bg = C.success+"20"; border = C.success; color = C.success; }
          else if (i === sel) { bg = C.error+"20"; border = C.error; color = C.error; }
        } else if (i === sel) { bg = C.accentSoft; border = C.accent; color = C.accent; }
        return (
          <button key={i} onClick={() => !checked && !disabled && setSel(i)} style={{ display:"block", width:"100%", textAlign:"left", padding:"10px 14px", marginBottom:6, background:bg, border:`1.5px solid ${border}`, borderRadius:10, color, cursor: (checked||disabled)?"default":"pointer", fontSize:13, transition:"all 0.2s", fontFamily:"'Segoe UI', sans-serif" }}>
            {checked && i===correct ? "✓ " : ""}{checked && i===sel && i!==correct ? "✗ " : ""}{String.fromCharCode(65+i)}. {opt}
          </button>
        );
      })}
      {!checked && !disabled && (
        <button onClick={() => { if (sel!==null) { setChecked(true); onResult && onResult(options[sel], options[correct], isCorrect); } }} disabled={sel===null}
          style={{ padding:"8px 18px", background: sel!==null?C.accent:C.surface, color: sel!==null?"#0f1923":C.textMuted, border:`1px solid ${C.accentBorder}`, borderRadius:8, cursor: sel!==null?"pointer":"default", fontWeight:700, fontSize:13, fontFamily:"'Segoe UI', sans-serif" }}>
          Check →
        </button>
      )}
      {checked && <div style={{ fontSize:13, color: isCorrect?C.success:C.error, marginTop:6 }}>{isCorrect?"✓ Correct!":` ✗ Correct answer: ${options[correct]}`}</div>}
    </div>
  );
}

// ─── TASK: Single True/False ──────────────────────────────────────────────────
    ...
    <div style={{ marginBottom: 14 }}>
      ...
      <div style={{ display:"flex", gap:10 }}>
        {[true, false].map(val => {
          ...
          if (checked) {
            if (val===correct) { bg=C.success+"20"; border=C.success; color=C.success; }
        ...
        {!checked && !disabled && sel!==null && (
          <button onClick={() => { setChecked(true); onResult && onResult(sel?"TRUE":"FALSE", correct?"TRUE":"FALSE", isCorrect); }} style={{ padding:"10px 16px", background:C.accent, color:"#0f1923", border:"none", borderRadius:10, fontWeight:700, fontSize:13, cursor:"pointer", fontFamily:"'Segoe UI', sans-serif" }}>Check →</button>
        )}
      </div>
      {checked && <div style={{ fontSize:12, color: isCorrect?C.success:C.error, marginTop:6 }}>{isCorrect?"✓ Correct!":` ✗ Correct answer: ${correct?"TRUE":"FALSE"}`}</div>}
    </div>
  );
}

// ─── TASK WRAPPERS (multi-question blocks) ────────────────────────────────────

// Multi True/False (paragraphs 1 & 4: 3 statements each)
function MultiTrueFalseTask({ task, onContinue }) {
  ...
  const handleResult = (idx, sa, ca, ic) => {
    setResults(prev => ({ ...prev, [idx]: { sa, ca, ic } }));
  };
  ...
  return (
    <div>
      ...
      {task.statements.map((s, i) => (
        <div key={i} style={{ background:C.surface, borderRadius:12, padding:"14px 16px", marginBottom:10, border:`1px solid ${C.accentBorder}` }}>
          <div style={{ fontSize:12, color:C.textMuted, marginBottom:6 }}>Statement {i+1}</div>
          <SingleTF statement={s.text} correct={s.correct} onResult={(sa,ca,ic) => handleResult(i,sa,ca,ic)} disabled={!!results[i]} />
        </div>
      ))}
      {allDone && (
        <div style={{ marginTop:14 }}>
          <div style={{ padding:"10px 14px", borderRadius:10, background:C.accentSoft, border:`1px solid ${C.accentBorder}`, fontSize:13, color:C.text, marginBottom:12 }}>
            ✅ All {total} statements answered! Score: {Object.values(results).filter(r=>r.ic).length}/{total}
          </div>
          <Btn onClick={() => onContinue(
            Object.values(results).map(r=>r.sa).join(" | "),
            Object.values(results).map(r=>r.ca).join(" | "),
            Object.values(results).every(r=>r.ic)
...
// Multi MCQ (paragraph 2: 3 questions)
      ...
      {task.questions.map((q, i) => (
        <div key={i} style={{ background:C.surface, borderRadius:12, padding:"14px 16px", marginBottom:10, border:`1px solid ${C.accentBorder}` }}>
          <SingleMCQ question={`${i+1}. ${q.question}`} options={q.options} correct={q.correct}
            onResult={(sa,ca,ic) => setResults(prev=>({...prev,[i]:{sa,ca,ic}}))}
            disabled={!!results[i]} />
        </div>
      ))}
      {allDone && (
        <div style={{ marginTop:14 }}>
          <div style={{ padding:"10px 14px", borderRadius:10, background:C.accentSoft, border:`1px solid ${C.accentBorder}`, fontSize:13, color:C.text, marginBottom:12 }}>
            ✅ All {total} questions answered! Score: {Object.values(results).filter(r=>r.ic).length}/{total}
          </div>
          <Btn onClick={() => onContinue(
            Object.values(results).map(r=>r.sa).join(" | "),
            Object.values(results).map(r=>r.ca).join(" | "),
...
function GapFillTask({ task, onContinue }) {
  ...
  const handleSelect = (sentIdx, word) => {
    ...
    if (answers[sentIdx] === word) {
      setAnswers(prev => { const n={...prev}; delete n[sentIdx]; return n; });
      setUsedWords(prev => prev.filter(w=>w!==word));
    } else {
      if (answers[sentIdx]) setUsedWords(prev=>prev.filter(w=>w!==answers[sentIdx]));
      setAnswers(prev => ({...prev,[sentIdx]:word}));
      setUsedWords(prev=>[...prev.filter(w=>w!==word),word]);
    }
  };
  ...
  return (
    <div>
      ...
      <div style={{ background:C.surface, borderRadius:12, padding:"12px 16px", marginBottom:14, fontSize:13, color:C.text }}>
        📖 Word Bank: {task.wordbank.map(w=>(
          <span key={w} style={{ display:"inline-block", margin:"3px 4px", padding:"3px 10px", borderRadius:20, background: usedWords.includes(w)?C.card:C.accentSoft, border:`1px solid ${usedWords.includes(w)?C.accentBorder:C.accent}`, color: usedWords.includes(w)?C.textMuted:C.accent, fontWeight:600, fontSize:12 }}>{w}</span>
        ))}
      </div>
      {task.sentences.map((s, i) => {
        ...
        return (
          <div key={i} style={{ background:C.surface, borderRadius:12, padding:"14px 16px", marginBottom:10, border:`1px solid ${checked?(isCorrect?C.success+"40":C.error+"40"):C.accentBorder}` }}>
            <div style={{ fontSize:14, color:C.text, lineHeight:1.6, marginBottom:8 }}>
              {i+1}. {parts[0]}
              <span style={{ display:"inline-block", minWidth:100, borderBottom:`2px solid ${checked?(isCorrect?C.success:C.error):C.accent}`, margin:"0 6px", padding:"0 6px", color:checked?(isCorrect?C.success:C.error):C.accent, fontWeight:700 }}>
                {userAns||"_____"}
              </span>
              {parts[1]}
            </div>
            {!checked && (
              <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
                {task.wordbank.filter(w=>!usedWords.includes(w)||answers[i]===w).map(w=>(
                  <button key={w} onClick={()=>handleSelect(i,w)} style={{ padding:"5px 12px", background: answers[i]===w?C.accentSoft:C.card, border:`1px solid ${answers[i]===w?C.accent:C.accentBorder}`, borderRadius:20, color: answers[i]===w?C.accent:C.textMuted, fontSize:12, fontWeight:600, cursor:"pointer", fontFamily:"'Segoe UI', sans-serif" }}>{w}</button>
                ))}
              </div>
            )}
            {checked && <div style={{ fontSize:12, color: isCorrect?C.success:C.error }}>{isCorrect?"✓ Correct!":` ✗ Correct answer: ${s.answer}`}</div>}
          </div>
        );
      })}
      {!checked && allFilled && <Btn onClick={handleCheck} style={{ marginTop:8 }}>Check All Answers →</Btn>}
      {checked && (
        <div style={{ marginTop:14 }}>
          <div style={{ padding:"10px 14px", borderRadius:10, background:C.accentSoft, border:`1px solid ${C.accentBorder}`, fontSize:13, color:C.text, marginBottom:12 }}>
            ✅ Score: {score}/{total}
          ...
          <Btn onClick={() => onContinue(
            task.sentences.map((s,i)=>answers[i]||"—").join(" | "),
            task.sentences.map(s=>s.answer).join(" | "),
            score===total
          )} style={{ width:"100%" }}>Continue →</Btn>
        </div>
      )}
...
// Mixed paragraph 5: 2 MCQ + 1 short answer
  ...
  return (
    <div>
      ...
      {task.questions.map((q,i)=>(
        <div key={i} style={{ background:C.surface, borderRadius:12, padding:"14px 16px", marginBottom:10, border:`1px solid ${C.accentBorder}` }}>
          <SingleMCQ question={`${i+1}. ${q.question}`} options={q.options} correct={q.correct}
            onResult={(sa,ca,ic)=>setMcqResults(prev=>({...prev,[i]:{sa,ca,ic}}))}
            disabled={!!mcqResults[i]} />
        </div>
      ))}
      {mcqDone && (
        <div style={{ background:C.surface, borderRadius:12, padding:"14px 16px", marginBottom:10, border:`1px solid ${C.accentBorder}` }}>
          ...
          <textarea value={shortAnswer} onChange={e=>setShortAnswer(e.target.value)} disabled={shortSubmitted}
            ...
            style={{ width:"100%", minHeight:80, background:C.card, border:`1.5px solid ${shortSubmitted?C.success+"60":C.accentBorder}`, borderRadius:10, padding:12, color:C.text, fontFamily:"'Segoe UI', sans-serif", fontSize:14, resize:"vertical", outline:"none", boxSizing:"border-box" }}/>
      ...
      {allDone && (
        <div style={{ marginTop:14 }}>
          ...
          <Btn onClick={() => onContinue(
            [...Object.values(mcqResults).map(r=>r.sa), shortAnswer].join(" | "),
            [...Object.values(mcqResults).map(r=>r.ca), "(open)"].join(" | "),
            Object.values(mcqResults).every(r=>r.ic)
          )} style={{ width:"100%" }}>Continue →</Btn>
        </div>
      )}
    </div>
...
function StudentPanel({ user, onLogout }) {
  ...
  const saveTaskResult = async (stageLabel, taskLabel, question, studentAnswer, correctAnswer, isCorrect) => {
    if (!attemptId) return;
    await supabase.from("task_results").insert({ user_id:user.id, attempt_id:attemptId, lesson_title:lesson.title, stage:stageLabel, task_label:taskLabel, question, student_answer:studentAnswer, correct_answer:correctAnswer, is_correct:isCorrect, answered_at:new Date().toISOString() });
  };

  const handlePreCheckContinue = async (sa, ca, ic) => {
    await saveTaskResult("pre","Pre-Reading Check",lesson.preCheck.question,sa,ca,ic);
    setStage(1);
  };

  const handleParaContinue = async (paraIdx, sa, ca, ic) => {
    const para = lesson.paragraphs[paraIdx];
    await saveTaskResult("while", para.task.label, para.label+" task", sa, ca, ic);
    showToast(ic ? "🎉 Well done!" : "📖 Keep going!");
  ...
  const restartLesson = async () => {
    setStage(0); setPreStep(0); setParaIndex(0); setParasDone([]); setPostAnswers({}); setPostSubmitted({}); setAttemptId(null);
    const { data } = await supabase.from("attempts").insert({ user_id:user.id, lesson_title:lesson.title, started_at:new Date().toISOString() }).select().single();
    if (data) setAttemptId(data.id);
  };
          ...
          <div>
            ...
            {preStep===0 && (
              <>
                ...
                <p style={{ color:C.textMuted,fontSize:13,marginBottom:18 }}>Tap a card to see the meaning. Press 🔊 Listen to hear the pronunciation.</p>
            ...
            {preStep===1 && (
              <Card>
                ...
                <SingleMCQ question={lesson.preCheck.question} options={lesson.preCheck.options} correct={lesson.preCheck.correct}
                  onResult={(sa,ca,ic)=>{}} disabled={false}/>
                <div style={{ marginTop:14 }}>
                  <Btn onClick={()=>handlePreCheckContinue("(skipped)","Coral reefs",false)} color={C.stage1} style={{ width:"100%" }}>Continue to Reading →</Btn>
                </div>
              </Card>
            )}
          </div>
                ...
                <div key={i}>
                  <Card>
                    <div style={{ display:"flex",gap:14,alignItems:"flex-start" }}>
                      ...
                      <div>
                        <div style={{ fontSize:11,color:C.textMuted,marginBottom:6,textTransform:"uppercase",letterSpacing:"0.06em" }}>{para.label}</div>
    ...
    <div style={{ minHeight:"100vh",background:C.bg,color:C.text,fontFamily:"'Segoe UI', sans-serif" }}>
      ...
      <div style={{ display:"flex",height:"calc(100vh - 65px)" }}>
        ...
        <div style={{ flex:1,overflowY:"auto",padding:"28px 32px" }}>
          {!selected && <div style={{ textAlign:"center",padding:60,color:C.textMuted }}><div style={{ fontSize:48,marginBottom:16 }}>👈</div><div style={{ fontSize:16 }}>Select a student to see their attempts</div></div>}
              ...
              {attempts.map((att,ai)=>(
                <div key={att.id} style={{ background:C.surface,borderRadius:20,marginBottom:28,border:`1px solid ${C.accentBorder}`,overflow:"hidden" }}>
                  <div style={{ padding:"16px 24px",background:C.card,borderBottom:`1px solid ${C.accentBorder}`,display:"flex",alignItems:"center",gap:16 }}>
                    ...
                    <div style={{ flex:1 }}><div style={{ fontWeight:800,fontSize:16,color:C.text }}>Attempt #{attempts.length-ai}</div><div style={{ fontSize:12,color:C.textMuted }}>{fmt(att.started_at)}</div></div>
                    {scoreOf(att) && <div style={{ textAlign:"right" }}><div style={{ fontSize:11,color:C.textMuted,marginBottom:2 }}>Test score</div><div style={{ fontSize:20,fontWeight:800,color:C.accent }}>{scoreOf(att)}</div></div>}
                  </div>
